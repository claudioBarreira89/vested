...
contract TokenVesting is Context, AccessProtected, ReentrancyGuard {
    ...
    */
	...
	struct Claim {
		...
		uint112 cliffAmount;
		bool isActive; // if false then revoked
    ...
    */
     ...
     */
	function reduceVesteeCliffTime(
		address vesteeAddress
	) external onlyAdmin nonReentrant {
		Claim storage userClaim = claims[vesteeAddress];

		uint40 durationOfVesting = userClaim.endTimestamp -
			userClaim.startTimestamp;
		uint40 onePercentOfVestingDuration = durationOfVesting / 100;

		// Subtract 1 percent from the original timestamp
		uint40 newEndTimestamp = userClaim.endTimestamp -
			onePercentOfVestingDuration;
