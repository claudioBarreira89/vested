...
export function CollectionDetails({ collection }: { collection: ERC721DropProviderState }) {
  ...
  return (
    <Well px="x5" py="x5" gap="x3">
      ...
      <CollectionDetailsItem
        ...
        value={shortAddress}
        href={`https://${NETWORK_NAME ? NETWORK_NAME.toLowerCase() + "." : ""}etherscan.io/token/${collection.address}`}
      />
      ...
      {collection.editionMetadata?.imageURI && (
        <CollectionDetailsItem name="IPFS imageURI" href={transformIPFSURL(collection.editionMetadata?.imageURI)} />
      )}
      <CollectionDetailsItem name="Etherscan transaction" href={`https://etherscan.io/tx/${collection.created.id}`} />
      <CollectionDetailsItem name="Resale royalty" value={`${collection.contractConfig?.royaltyBPS / 100}%`} />
    </Well>
  );
}
