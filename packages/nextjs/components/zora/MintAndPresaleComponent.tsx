...
export function MintAndPresaleComponent({
  ...
  const { presaleExists, merkleRootExists, saleNotStarted, saleIsFinished } = useSaleStatus({
    collection,
  });

  const [showPresale, setShowPresale] = useState(saleNotStarted && !saleIsFinished && showPresaleValue);
