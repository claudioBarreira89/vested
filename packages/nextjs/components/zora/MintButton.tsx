...
export function MintButton({
  ...
  const { startDate, endDate, isSoldOut, saleIsActive, saleNotStarted, saleIsFinished } = useSaleStatus({
    collection,
    presale,
  });
  ...
  if (saleIsFinished || isSoldOut) {
    return (
      <Box>
        ...
        <Paragraph mt="x1" textAlign="center" size="sm" color="secondary" maxW="x64" mx="auto">
  ...
  return (
    <>
      <Button
        ...
        onClick={!address ? openConnectModal : chain?.unsupported ? openChainModal : handleMint}
        style={isMinted ? { backgroundColor: vars.color.positive, color: vars.color.onPositive } : {}}
      ...
      {saleNotStarted && (
        <Text variant="paragraph-sm" textAlign="center" color="tertiary">
          <CountdownTimer targetTime={startDate} refresh={true} prependText="Starts in " />
        </Text>
      )}
      {errors && (
