...
function AssetRenderer({ mediaType = MediaType.UNKNOWN, ...props }: AssetRendererProps) {
  ...
  const accessor = useMemo(() => lowerCase(mediaType) as "image" | "video" | "audio" | "unknown", [mediaType]);

  // Remove props that are not valid for the component type
  const filteredProps = useMemo(() => {
    return mediaType == MediaType.IMAGE ? omit(props, ["coverImageUrl", "image", "compact", "collection"]) : props;
  }, [mediaType, props]);

  const compProps = useMemo(() => props?.[accessor], [accessor, props]);
  return <Component {...filteredProps} {...compProps} />;
}
