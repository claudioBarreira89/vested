...
export function MintComponent({
  ...
  const maxPerWallet = parseInt(
    presale ? allowlistEntry?.maxCanMint || "0" : collection.salesConfig.maxSalePurchasePerAddress.toString(),
  );
  ...
  return (
    <Stack className={[className, "zord-mint-status", presale && "zord-mint-status--presale"]} gap="x4" {...props}>
        ...
        <Flex gap="x3" flexChildren justify="space-between" align="flex-end" wrap="wrap">
          <Stack gap="x1" style={{ flex: "none" }}>
            ...
            <Heading size="sm" className={priceDateHeading}>
              {internalPrice === "0" ? "Free" : `${formatCryptoVal(Number(internalPrice) * (mintCounter || 1))} ETH`}
            </Heading>
          </Stack>

          {saleIsActive && !isSoldOut ? (
            <Stack gap="x1" style={{ textAlign: "right" }}>
              <Flex className={"zord-mint-status__number-input"} gap="x1" ml="auto" justify="flex-end" align="center">
                <Button
                  w="x12"
                  variant="circle"
                  disabled={mintCounter <= 1}
                  onClick={() => handleMintCounterUpdate((state: number) => (state > 0 ? state - 1 : state))}
                >
                  <Heading size="sm" className={priceDateHeading}>
                    –
                  </Heading>
                ...
                <Button
                  ...
                  variant="circle"
                  onClick={() => setMintCounter(state => (state < maxPerWallet ? state + 1 : state))}
                >
                  <Heading size="sm" className={priceDateHeading}>
                    +
                  </Heading>
