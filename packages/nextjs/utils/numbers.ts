...
export function amountFormatter(
  ...
  const parsedAmount = new BigNumber(raw).shiftedBy(baseDecimals > 0 ? baseDecimals * -1 : 0).dp(displayDecimals);
...
export function formatCryptoVal(cryptoVal: BigNumber | BigNumberish | string, baseDecimals = 18) {
  const raw = typeof cryptoVal === "string" ? cryptoVal : cryptoVal.toString();
  const parsedamount = new BigNumber(raw).shiftedBy(baseDecimals > 0 ? baseDecimals * -1 : 0);
