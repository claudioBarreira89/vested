...
export function useSaleStatus({
  ...
  const isSoldOut = collection.maxSupply && collection.totalMinted && collection.totalMinted >= collection.maxSupply;
  ...
  const merkleRootExists = collection.salesConfig?.presaleMerkleRoot !== HashZero.toString();

  const presaleIsActive = merkleRootExists && presaleStart <= Date.now() && presaleEnd > Date.now();
