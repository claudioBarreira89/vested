...
import { transformIPFSURL } from "./IPFSProvider";
...
export interface DropMetadataProviderState {
  ...
  updateMetadataBase: (baseUri: string, newContractUri: string) => Promise<ContractTransaction | undefined>;
}

export const DropMetadataContext = React.createContext<DropMetadataProviderState>({} as DropMetadataProviderState);
...
function DropMetadataContractProvider({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { base, extension, contractURI } = await metadataRenderer.metadataBaseByContract(collection.address);
  ...
  const updateMetadataBase = useCallback(
    async (baseUri: string, newContractUri: string) => {
      ...
      const tx = await metadataRenderer.updateMetadataBase(collection.address, baseUri, newContractUri);
      await tx.wait(2);
