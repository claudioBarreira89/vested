...
export interface ERC721DropProviderState
  extends Omit<SubgraphERC721Drop, "salesConfig" | "totalMinted" | "maxSupply" | "owner" | "fundsRecipient"> {
  loading: boolean;
  purchase: (quantity: number) => Promise<ContractTransaction | undefined>;
  purchasePresale: (quantity: number, allowlistEntry?: AllowListEntry) => Promise<ContractTransaction | undefined>;
...
export const ERC721DropContext = React.createContext<ERC721DropProviderState>({} as ERC721DropProviderState);
...
function ERC721DropContractProvider({
  ...
  const defaultEditionSalesDetails = {
    ...
    maxSalePurchasePerAddress: BigNumber.from(collection.salesConfig.maxSalePurchasePerAddress),
  ...
  const purchase = useCallback(
    async (quantity: number) => {
      ...
      const tx = await drop.purchase(quantity, {
        value: (state.salesConfig.publicSalePrice as BigNumber).mul(BigNumber.from(quantity)),
      });
      return tx;
    },
  ...
  const userCanMint = async () => {
    ...
    return (
      (await drop.mintedPerAddress(address)).totalMints.lt(state.salesConfig.maxSalePurchasePerAddress) &&
