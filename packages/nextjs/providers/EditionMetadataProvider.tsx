...
export interface EditionMetadataProviderState {
  ...
  updateMediaURIs: (imageURI: string, animationURI: string) => Promise<ContractTransaction | undefined>;
}
...
function EditionMetadataContractProvider({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const metadata = (await metadataRenderer.tokenInfos(collection.address)) as MetadataDetails;
        setMetadataDetails({
          ...metadata,
  ...
  const updateDescription = useCallback(
    async (description: string) => {
      ...
      const tx = await metadataRenderer.updateDescription(collection.address, jsonDescription);
      console.log({ tx });
      await tx.wait(2);
  ...
  const updateMediaURIs = useCallback(
    async (imageURI: string, animationURI: string) => {
      ...
      const tx = await metadataRenderer.updateMediaURIs(collection.address, imageURI, animationURI);
      await tx.wait(2);
